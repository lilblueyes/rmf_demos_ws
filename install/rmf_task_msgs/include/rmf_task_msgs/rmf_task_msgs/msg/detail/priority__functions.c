...
rmf_task_msgs__msg__Priority__create(void)
{
  ...
  rmf_task_msgs__msg__Priority * msg = (rmf_task_msgs__msg__Priority *)allocator.allocate(sizeof(rmf_task_msgs__msg__Priority), allocator.state);
  if (!msg) {
    return NULL;
  }
...
rmf_task_msgs__msg__Priority__Sequence__init(rmf_task_msgs__msg__Priority__Sequence * array, size_t size)
{
  ...
  if (size) {
    data = (rmf_task_msgs__msg__Priority *)allocator.zero_allocate(size, sizeof(rmf_task_msgs__msg__Priority), allocator.state);
    if (!data) {
      return false;
    }
...
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  rmf_task_msgs__msg__Priority__Sequence * array = (rmf_task_msgs__msg__Priority__Sequence *)allocator.allocate(sizeof(rmf_task_msgs__msg__Priority__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
...
rmf_task_msgs__msg__Priority__Sequence__are_equal(const rmf_task_msgs__msg__Priority__Sequence * lhs, const rmf_task_msgs__msg__Priority__Sequence * rhs)
{
  ...
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!rmf_task_msgs__msg__Priority__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
  ...
  rmf_task_msgs__msg__Priority__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(rmf_task_msgs__msg__Priority);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    rmf_task_msgs__msg__Priority * data =
      (rmf_task_msgs__msg__Priority *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!rmf_task_msgs__msg__Priority__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          rmf_task_msgs__msg__Priority__fini(&output->data[i]);
        }
        return false;
      }
    }
