...
bool rmf_ingestor_msgs__msg__ingestor_request__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("rmf_ingestor_msgs.msg._ingestor_request.IngestorRequest", full_classname_dest, 55) == 0);
  }
  rmf_ingestor_msgs__msg__IngestorRequest * ros_message = _ros_message;
  {  // time
  ...
  {  // items
    ...
    if (!rmf_ingestor_msgs__msg__IngestorRequestItem__Sequence__init(&(ros_message->items), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create rmf_ingestor_msgs__msg__IngestorRequestItem__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    rmf_ingestor_msgs__msg__IngestorRequestItem * dest = ros_message->items.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!rmf_ingestor_msgs__msg__ingestor_request_item__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of IngestorRequest */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("rmf_ingestor_msgs.msg._ingestor_request");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "IngestorRequest");
  ...
  {  // items
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = rmf_ingestor_msgs__msg__ingestor_request_item__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
