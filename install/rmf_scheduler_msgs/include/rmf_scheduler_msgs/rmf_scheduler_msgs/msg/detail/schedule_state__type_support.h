// generated from rosidl_generator_c/resource/idl__type_support.h.em
// with input from rmf_scheduler_msgs:msg/ScheduleState.idl
// generated code does not contain a copyright notice

// IWYU pragma: private, include "rmf_scheduler_msgs/msg/schedule_state.h"


#ifndef RMF_SCHEDULER_MSGS__MSG__DETAIL__SCHEDULE_STATE__TYPE_SUPPORT_H_
#define RMF_SCHEDULER_MSGS__MSG__DETAIL__SCHEDULE_STATE__TYPE_SUPPORT_H_

#include "rosidl_typesupport_interface/macros.h"

#include "rmf_scheduler_msgs/msg/rosidl_generator_c__visibility_control.h"

#ifdef __cplusplus
extern "C"
{
#endif

#include "rosidl_runtime_c/message_type_support_struct.h"

// Forward declare the get type support functions for this type.
ROSIDL_GENERATOR_C_PUBLIC_rmf_scheduler_msgs
const rosidl_message_type_support_t *
ROSIDL_TYPESUPPORT_INTERFACE__MESSAGE_SYMBOL_NAME(
  rosidl_typesupport_c,
  rmf_scheduler_msgs,
  msg,
  ScheduleState
)(void);

#ifdef __cplusplus
}
#endif

#endif  // RMF_SCHEDULER_MSGS__MSG__DETAIL__SCHEDULE_STATE__TYPE_SUPPORT_H_
